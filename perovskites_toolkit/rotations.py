...
def move_xyz_coords(
    ...
    if not halide_site:
        halide_element = [
            element
            for element in structure.composition.as_dict()
            if element in ["F", "Cl", "Br", "I"]
        ][0]
        halide_sites = [
            site for site in structure if site.species_string == halide_element
        ]
        halide_site = sorted(
            sorted(
                sorted(halide_sites, key=lambda site: site.coords[2]),
...
def get_center_of_mass(structure: Structure, sites_indexes: list) -> np.array:
    """
...
def get_a_cation_sites(
    ...
    carbon_sites = [
        ...
        if site.species_string == a_cation_center_species
    ]
    fa_molecules = []
    count = 0
    for index, carbon in carbon_sites:
        ...
        print(
            [site.species_string for site in neighbors]
        )  # Make sure we're getting the right atoms
        fa_molecules.append(
            [
                index,
            ]
