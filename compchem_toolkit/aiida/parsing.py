...
from pymatgen.io.vasp.outputs import BSVasprun, Chgcar, Outcar, Vasprun
...
def get_vasprun_from_pk(
    pk: int, remove_file: bool = True, vasprun_type: str = "Vasprun"
    ...
    if computer:
        subprocess.run(
            [
                "rsync",
                "-azvu",
                f"{computer}:{remote_path}/vasprun.xml",
                f"./vaspruns/vasprun_{label}.xml",
            ]
        )
...
def transfer_chgcar(
    ...
    """
    node = load_node(pk)
    assert node.outputs.remote_folder.attributes[
        "remote_path"
    ], f"No remote path for pk {pk}"
    remote_path = node.outputs.remote_folder.attributes["remote_path"]
    ...
    shell_output = subprocess.run(
        [
            "rsync",
            "-azvus",
            f"{remote_computer}:{remote_path}/CHGCAR",
            f"{abs_path}/CHGCARS/{node_label}_CHGCAR",
        ]
    )
...
def transfer_wavecar(
    ...
    shell_output = subprocess.run(
        [
            "rsync",
            "-azvus",
            f"{remote_computer}:{remote_path}/WAVECAR",
            f"{abs_path}/WAVECARS/{node_label}_WAVECAR",
        ]
    )
    if shell_output.returncode == 0:
    ...
    for vasp_file in list_of_files:
        ...
        shell_output = subprocess.run(
            [
                "rsync",
                "-azvus",
                f"{remote_computer}:{remote_path}/{vasp_file}",
                f"{file_path}",
            ]
        )
        if shell_output.returncode == 0:
