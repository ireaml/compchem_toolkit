...
def get_selective_dynamics_from_layers(
    structure: Structure, number_of_layers_in_slab: int, epsilon: float = 0.01
):
    """
    ...
    max_z = max(z_coords)
    print(f"Max z of site in slab: {max_z:.4f}")
    min_z = min(z_coords)
    print(f"Min z of site in slab: {min_z:.4f}")
    ...
    for atom in structure_copy:
        ...
        if atom.coords[2] > (max_z - (height_per_layer + epsilon)) or atom.coords[2] < (
            min_z + (height_per_layer + epsilon)
        ):
            atoms_in_outer_layers.append(atom)
            selective_dynamics["positions_dof"].append(
                [True, True, True]
            )  # allow to relax
        else:
            selective_dynamics["positions_dof"].append(
                [False, False, False]
            )  # dont relax. Fix their coordinates

    print(f"Total number of atoms in structure: {len(structure_copy)}")
    print(
        f"Number of atoms in outer layers (will be relaxed): {len(atoms_in_outer_layers)}"
    )
