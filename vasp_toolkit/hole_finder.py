...
def hole_finder(
    ...
    for k in range(len(eigen_up)): # loop over kpoint
        for b in range(len(eigen_up[k])): # loop over band
            # Locate VBM
            if eigen_up[k][b][0] == defect_data.eigenvalue_band_properties[2] or \
                eigen_down[k][b][0] == defect_data.eigenvalue_band_properties[2]:
    ...
    for b in range(vbm_data[0] - 6, vbm_data[0]+6): # check bands around VBM
        for k in range(len(eigen_up)): # loop over kpoint
            eigenvals.append([b+1, k+1, eigen_up[k][b][0], eigen_down[k][b][0],eigen_up[k][b][1],eigen_down[k][b][1]])
    df = pd.DataFrame(eigenvals,
                        columns=["band", "k-point", "up-eigenval", "down-eigenval", "up-occupancy", "down-occupancy"])
    ...
    elif hole_spin_up.empty == True and hole_spin_down.empty == False: # holes only on the spin up channel
        print(f"Found {hole_spin_down/num_kpts} hole in the spin down channel:\n", hole_spin_down)
        print("Recommended min and max for EINT in PARCHG calculation:", \
            round(hole_spin_down["down-eigenval"].min(), 2), round(hole_spin_down["down-eigenval"].max(), 2))
    elif hole_spin_down.empty == True and hole_spin_up.empty == False: # holes only on the spin down channel
        print(f"Found {hole_spin_up/num_kpts} hole in the spin up channel:\n", hole_spin_up)
        print("Recommended min and max for EINT in PARCHG calculation:", \
            round(hole_spin_up["up-eigenval"].min(), 2), round(hole_spin_up["up-eigenval"].max(), 2))
    
    elif hole_spin_up.empty == False and hole_spin_down.empty == False: # holes on both spin channels
        print(f"Found {hole_spin_up/num_kpts} holes in the spin up channel and {hole_spin_down/num_kpts} \
            in the spin down channel:\n", hole_spin_up, "\n", hole_spin_down)
        print("Holes in spin up: recommended min and max for EINT in PARCHG calculation:", \
                round(hole_spin_up["up-eigenval"].min(), 2), round(hole_spin_up["up-eigenval"].max(), 2))
        print("Holes in spin down: Recommended min and max for EINT in PARCHG calculation:", \
                round(hole_spin_down["down-eigenval"].min(), 2), round(hole_spin_down["down-eigenval"].max(), 2))
