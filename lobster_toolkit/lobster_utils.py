...
def get_number_of_bands(
    ...
    bands = 0
    assert type(composition) == pymatgen.core.composition.Composition, f"Composition must be a pymatgen.core.composition.Composition.\
...
def get_labels_by_elements(
    ...
    for label in all_labels:
        if all(element in [element_1, element_2] for element in [complete_cohp.bonds[str(label)]['sites'][0].species_string, complete_cohp.bonds[str(label)]['sites'][1].species_string]):
...
def plot_cohp_for_label_list(
    ...
    cp.get_plot(ylim=ylim, xlim=[xlim[0]/divisor, xlim[1]/divisor ], integrated=False)
    # Get figure
    fig = plt.gcf()
    ...
    legend = ax.get_legend(); legend.remove()

    # Check if spin polarised calc
    if len(lines) == 3:
        line = lines[0]; line.set_c("tab:grey") # non spin polarised
        line.set_linewidth(1.8)
    else:
        lines[0].set_c("tab:grey") ; lines[0].set_linestyle("-") # 1 spin channel
        lines[1].set_c("tab:grey") ; lines[1].set_linestyle("--") # 2 spin channel
        [line.set_linewidth(1.8) for line in lines[0:2]]
        if spin_legend:
            plt.legend(
                [lines[0], lines[1]], ['Spin up', 'Spin down'], loc= 'upper left',
                fontsize = 14,
            )
    ...
    lines[-2].set_linewidth(1.8) ; lines[-2].set_c("tab:grey")
...
def plot_cohp_orb_res(
    ...
    for orb_key in complete_cohp.orb_res_cohp[label].keys():
        ...
        if abs(cohp_of_orb) > threshold:
            ...
            plot_label = (
                str(sites[0].species_string) + "(" + orb_1_fmt + ")-"
                + str(sites[1].species_string) + "(" + orb_2_fmt + ")"
            )
            cp.add_cohp(
                plot_label, complete_cohp.get_orbital_resolved_cohp(label=label, orbitals=orbitals)
            )
    ...
    plt.annotate(
        r"Bonding", ha="right", color="k", xy=(0.95, 0.05),
        xycoords='axes fraction',
        fontsize = fontsize_annot,
        bbox=dict(boxstyle="round", fc="tab:grey", alpha=0.15)
    )
    plt.annotate(
        r"Anti-Bonding", ha="left", color="k", xy=(0.05, 0.05),
        xycoords='axes fraction',
        fontsize = fontsize_annot,
        bbox=dict(boxstyle="round", fc="tab:grey", alpha=0.15)
    )
    ...
    # Reduce linewidth of line at Efermi and Line at x=0
    lines[-1].set_linewidth(1.2)
    lines[-2].set_linewidth(1.5) ; lines[-2].set_c("tab:grey")
    ...
    plt.ylabel("$E$ - $E_f$ (eV)",  fontsize = axis_label_size)
    plt.locator_params(axis='x', nbins=5) ; plt.locator_params(axis='y', nbins=7)
    plt.xticks(fontsize= 19) ; plt.yticks(fontsize= 16)
